...
```toml
batch_size  = 2     # sequences per microbatch (per GPU forward pass)
grad_accum  = 32    # microbatches before one optimizer step
...
Why `batch_size = 2`: the RWKV-7 WKV scan in
...
`batch_size × max_seq_len × num_layers`, and on a 24 GB L4 with the
16-layer 50M micro at `max_seq_len = 512`, `batch_size = 2` is the largest
microbatch that reliably fits.
