...
use std::sync::Arc;
use tiny_bit_core::config::ModelConfig;
use tiny_bit_infer::{engine::InferenceEngine, session::Session};
...
async fn list_models(State(state): State<Arc<AppState>>) -> Json<ModelsResponse> {
    Json(ModelsResponse {
        object: "list",
        data: vec![ModelInfo { id: "tiny-bit-small".to_string(), object: "model" }],
    })
}
...
pub async fn run(args: ServeArgs) -> anyhow::Result<()> {
    ...
    println!("tiny-bit server listening on http://{addr}");
